...
def initialize(args):
    ...
    package_path = os.path.join(args.output_directory, 'init') \
        if args.output_directory is not None else 'init'
    ...
    with open("uluru/templates/schema/schema_sample_cat.json") as template:
        with open(os.path.join(package_path, "out_schema_sample.json"), "w") as destination:
