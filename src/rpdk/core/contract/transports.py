...
class LocalLambdaTransport:
    ...
    def __call__(self, payload, callback_endpoint):
        """Sends the payload to the local lambda endpoint.

        If callback_endpoint is ``None``, the operation is assumed to be
        synchronous, and the callback endpoint will not be set for the request.
        """
        try:
