...
def get_cloudformation_exports(region_name, endpoint_url, role_arn, profile_name, headers):
    session = create_sdk_session(region_name, profile_name)
    temp_credentials = get_temporary_credentials(session, role_arn=role_arn, headers=headers)
    ...
    try:
        with path.open("r", encoding="utf-8") as f:
            overrides_raw = render_template(
                f.read(), region_name, endpoint_url, role_arn, profile_name, headers=headers
            )
...
# flake8: noqa: C901
def get_hook_overrides(root, region_name, endpoint_url, role_arn, profile_name, headers):
    if not root:
