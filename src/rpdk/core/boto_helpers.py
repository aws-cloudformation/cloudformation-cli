...
def get_temporary_credentials(session, key_names=BOTO_CRED_KEYS, role_arn=None, headers=None):
    sts_client = session.client(
        "sts",
        endpoint_url=get_service_endpoint("sts", session.region_name),
        region_name=session.region_name,
    )
    check_keys=set(["account_id","source_arn"])
    if headers and check_keys.issubset(headers.keys()) and headers["account_id"] and headers["source_arn"]:
